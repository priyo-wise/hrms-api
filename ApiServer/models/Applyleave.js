...
const create = async (Leave) => {
  const countleave = await db.query(
    `SELECT SUM(leavetransactions.NoOfLeaveDebited)-SUM(leavetransactions.NoOfLeaveCredited) as Balance, SUM(leavetransactions.NoOfLeaveCredited) as Usedleave FROM employeeleavetransactions leavetransactions where leavetransactions.EmployeeId=${Leave.EmployeeId} AND leavetransactions.leavetypeId=${Leave.LeaveTypeId} GROUP BY leavetransactions.EmployeeId,leavetransactions.leavetypeId`
  );
  const Allocatedleave = await db.query(
    `SELECT SUM(empleavetransaction.NoOfLeaveDebited) as Allocatedleave,empleavetransaction.* FROM employeeleavetransactions empleavetransaction WHERE empleavetransaction.leavetypeId=${Leave.LeaveTypeId} AND empleavetransaction.EmployeeId=${Leave.EmployeeId}`
  );
  const leaveBalance = await db.query(
    `SELECT t1.Balance FROM employeeleavebalances t1 INNER JOIN employeeleaves t2 on t1.LeaveTypeId=t2.LeaveTypeId where t2.EmployeeId=${Leave.EmployeeId} AND t1.LeaveTypeId=${Leave.LeaveTypeId}`
  );
  const useleave = await db.query(
    `SELECT DATEDIFF(t2.LeaveToDate,t2.LeaveFromDate) as Usedleave FROM employeeleaves t2 where t2.EmployeeId=${Leave.EmployeeId} AND t2.LeaveTypeId=${Leave.LeaveTypeId}`
  );
  const Allocatedleave0 = await db.query(
    `SELECT Balance as AllocatedleaveBalance FROM employeeleavebalances  where EmployeeId=${Leave.EmployeeId} AND LeaveTypeId=${Leave.LeaveTypeId}`
  );
  const currentleavecount = await db.query(
    `SELECT DATEDIFF('${Leave.LeaveToDate}','${Leave.LeaveFromDate}') as currentleavecount FROM employeeleaves where EmployeeId=${Leave.EmployeeId} AND LeaveTypeId=${Leave.LeaveTypeId}`
  );
  const rows = await db.query(
    `SELECT *  FROM employeeleaves  where EmployeeId = ${Leave.EmployeeId} and LeaveFromDate ='${Leave.LeaveFromDate}'`
  );
  const Usedleave = useleave == "" ? 0 : useleave[0].Usedleave;
  const currentleave =
    currentleavecount == "" ? 0 : currentleavecount[0].currentleavecount;

  const getAllocatedleave =
    Allocatedleave == "" ? 0 : Allocatedleave[0].leaveTransactionsId;
  ...
  if (countleave.length == 0 || Leave.LeaveTypeId == "8") {
    if ((Leave.LeavesId || 0) == 0) {
      if (rows.length > 0) {
        ...
        values ('${Leave.EmployeeId}','${currentdate.toISOString()}', '${
          Leave.LeaveFromDate
        }','${Leave.LeaveToDate}', '${Leave.LeaveTypeId}',
        1,'${Leave.Remarks}',5)`;
  ...
  if (getAllocatedleave == 0) {
    ...
    countleave[0].Balance == 0 ||
    countleave[0].Balance < countleave[0].Usedleave
  ) {
    ...
    if ((Leave.LeavesId || 0) == 0) {
      ...
      values ('${Leave.EmployeeId}','${Leave.LeaveTypeId}','${Allocatedleave[0].ValidFormDate}','${Allocatedleave[0].ValidToDate}','${Leave.LeaveFromDate}','${Leave.leavedays}','0','${Leave.Remarks}','${Leave.EmployeeId}','Cr')`;
        ...
        var addempleaveBlance = "update employeeleavebalances set";
        addempleaveBlance += ` Balance='${
          Allocatedleave[0].Allocatedleave -
          (currentleave == 0 ? 1 : currentleave)
        }'`;
        addempleaveBlance += ` where LeaveTypeId='${Leave.LeaveTypeId}' and EmployeeId='${Leave.EmployeeId}' `;
...
const getApprovedleave = async () => {
  const rows = await db.query(
    `SELECT count(ApprovalStatusId) as ApprovalStatus from employeeleaves e1 where e1.ApprovalStatusId=3`
  );
  // console.log(rows);
  return helper.emptyOrRows(rows);
};
...
const getRejectedleave = async () => {
  const rows = await db.query(
    `SELECT count(ApprovalStatusId) as RejectedStatus from employeeleaves e1 where e1.ApprovalStatusId=4`
  );
  //console.log(rows);
  return helper.emptyOrRows(rows);
};
...
const getAppliedleave = async () => {
  const rows = await db.query(
    `SELECT count(StatusId) as AppliedleaveStatus from employeeleaves e1 where e1.StatusId=1`
  );
  //console.log(rows);
  return helper.emptyOrRows(rows);
};
...
const getCancelleave = async () => {
  const rows = await db.query(
    `SELECT count(StatusId) as CancelLeaveStatus from employeeleaves e1 where e1.StatusId=2`
  );
  //console.log(rows);
  return helper.emptyOrRows(rows);
};
...
const updateleavestatus = async (Leave) => {
  const empleave = await db.query(
    `select ApprovalStatusId, StatusId from employeeleaves where LeaveFromDate='${Leave.LeaveFromDate}'`
  );
  ...
  if (
    empleave[0].ApprovalStatusId != "5" ||
    empleave[0].StatusId == "1" ||
    ((empleave[0].ApprovalStatusId != "4" ||
      empleave[0].ApprovalStatusId == "3") &&
      empleave[0].StatusId == "1")
  ) {
    if (Leave.LeavesId > 0) {
      query = "update employeeleaves set";
      query += ` ApprovalStatusId='${
        Leave.leavestatus == "2" ? 5 : Leave.leavestatus == "4" ? 4 : 3
      }'`;
