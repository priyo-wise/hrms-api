...
const AddEditErrorlog = (prop, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    openModal: async (id) => {
      const data = await WebService({
        endPoint: `Errorlog/FetchErrorlog/${id || 0}`,
        dispatch,
      });
      setData(data);
      setShow(true);
  ...
  const onSubmit = async (data) => {
    ...
    const responsedata = await WebService({
      endPoint: "Errorlog/SubmitErrorlog",
      body: data,
      dispatch,
    });
  ...
  return (
    <Modal show={show} onHide={handleClose}>
      <Modal.Header closeButton>
        <Modal.Title>
          {(data.Errorlog.ErrorId || 0) === 0 ? "Add Error" : "Edit Erro"}
        </Modal.Title>
      </Modal.Header>
      <Form
...
export default forwardRef(AddEditErrorlog);
