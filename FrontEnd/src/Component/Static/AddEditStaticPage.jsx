...
const AddEditStaticPage = (prop, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    openModal: async (id) => {
      const data = await WebService({
        endPoint: `StaticPage/Fetch/${id || 0}`,
        dispatch,
      });
  ...
  const schema = yup
    ...
    .shape({
      ...
      PageName: yup.string().trim().required(requiredMessage),
    })
    .required();
  const onSubmit = async (data) => {
    if (data.PageId === undefined) {
      await WebService({ endPoint: "StaticPage/Create", body: data, dispatch });
    } else {
      await WebService({
        endPoint: `StaticPage/Update/${data.PageId}`,
        method: "PUT",
        body: data,
        dispatch,
      });
        ...
        <div className="row col-md-12 p-3 m-2  mb-4">
          ...
          <div className="col-6 text-end mt-2">
            <Button
              ...
              id="btnStaticPageSubmit"
              type="submit"
            >
              {(data.PageId || 0) === 0 ? "Submit" : "Save Changes"}
            </Button>
          </div>
        </div>
      </Form>
...
export default forwardRef(AddEditStaticPage);
